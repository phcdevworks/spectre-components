...
describe('sp-card', () => {
  ...
  it('preserves nested interactive elements in projected content', async () => {
    const element = document.createElement('sp-card') as SpectreCardElement
    element.innerHTML =
      '<h2>Plan</h2><a href="/plan">View details</a><button type="button">Select</button>'
  ...
  it('respects padded="false" set via attribute', async () => {
    const element = document.createElement('sp-card') as SpectreCardElement
    element.setAttribute('padded', 'false')

    document.body.append(element)
    await element.updateComplete

    expect(element.padded).toBe(false)
    const div = element.querySelector('div')
    expect(div?.className).not.toContain('sp-card--padded')
  })

  it('treats any other padded attribute value as true', async () => {
    const element = document.createElement('sp-card') as SpectreCardElement
    element.setAttribute('padded', '')
  ...
  it('appends sanitized inner-class utility classes to the native div', async () => {
    ...
    const div = element.querySelector('div')
    expect(div?.className).toContain('sp-card__media--bleed')
